            ...
            {!auth ? <h2 style={{color:'white'}}>404 Error: Page not found</h2> : todo?.filter(item => item.id === id).map((item) => (
                <div>
                    ...
                    <Grid className="taskTitle">
                        <h1 style={{textDecoration:`${item.status ? "line-through" : "none"} `}} className="des">{item.title}</h1>
                        <Paper style={{ padding: '5px 10px' }} className="subtaskEditor">
                            <AlertDialog label={<AiOutlineEdit/>}
                    ...
                    <div className="taskTitle">
                        ...
                        <Paper style={{ padding: '5px 10px' }} className="subtaskEditor">
                            <AlertDialog label={<AiOutlineEdit/>} value={ `Previouse Text was: [ ${item.des} ]`}
                                name="des"
                                handleChange={handleChange}
                                handleSuccessMessage={handleSuccessMessage}
                            />
                    ...
                    <fieldset className="fieldset">
                        <legend style={{color:'black', fontSize:'20px'}}>Sub Tasks</legend>
                        {item.subtask?.map((sTask, index) => (
                            <div className='sub-task' key={index}>
                                ...
                                <Paper style={{background:'white'}} className="subtaskEditor">
                                    <AlertDialog label={<AiOutlineEdit/>} value={`Previouse Text was: [ ${sTask} ]`}
